...
changeBuildType(uuid("7e62a1a9-b045-4f9c-be42-cb9a649441e1_PreReleaseBuild")) {
    expectSteps {
        ...
        dotnetPack {
            name = "Pack Miruken.AspNet"
            projects = "Source/Miruken.AspNet/Miruken.AspNet.csproj"
        ...
        dotnetPack {
            name = "Pack Miruken.AspNet.Castle"
            projects = "Source/Miruken.AspNet.Castle/Miruken.AspNet.Castle.csproj"
            args = "-p:PackageVersion=%PackageVersion% -p:DebugSymbols=true -p:DebugType=pdbonly -p:Version=%DotNetAssemblyVersion% --include-symbols --include-source --no-build"
            param("dotNetCoverage.dotCover.home.path", "%teamcity.tool.JetBrains.dotCover.CommandLineTools.DEFAULT%")
        }
        dotnetPack {
            name = "Pack Miruken.AspNet.Swagger"
            projects = "Source/Miruken.AspNet.Swagger/Miruken.AspNet.Swagger.csproj"
            args = "-p:PackageVersion=%PackageVersion% -p:DebugSymbols=true -p:DebugType=pdbonly -p:Version=%DotNetAssemblyVersion% --include-symbols --include-source --no-build"
            param("dotNetCoverage.dotCover.home.path", "%teamcity.tool.JetBrains.dotCover.CommandLineTools.DEFAULT%")
        }
    }
